// ShotSync content script — Myer Supplier Portal
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type !== 'SHOTSYNC_PUSH_LISTING' || message.channel !== 'myer') return
  fillMyerListing(message.payload)
    .then(result => sendResponse(result))
    .catch(err => sendResponse({ success: false, error: err.message }))
  return true
})

async function fillMyerListing(payload) {
  // Field mapping TBD — requires access to Myer supplier portal to verify selectors
  showOverlay('Myer portal automation coming soon. Field mapping requires portal access.', 'info')
  return { success: false, error: 'Myer automation not yet configured' }
}
