  ...
  const jobMeta = (
    <div style={{ marginBottom: '28px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '6px' }}>
        <h1 className="text-[1.57rem] font-[600] tracking-[-0.5px] text-[var(--text)]">
          {job.name}
        </h1>
        <span className="chip chip-success">Exported</span>
      </div>
      <p className="text-[0.95rem] text-[var(--text3)]">
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
      {[
        ...
        <div key={label} className="card" style={{ padding: '16px 18px' }}>
          <p className="text-[0.87rem] text-[var(--text3)] mb-1">{label}</p>
          <p className={`text-[1.32rem] font-[600] tracking-[-0.3px] ${isAccent ? 'text-[var(--accent2)]' : 'text-[var(--text)]'}`}>{value}</p>
        </div>
      ))}
    </div>
  )
          ...
          {sessionSource === 'local' ? (
            <div className="card p-5 mb-6">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-[1.02rem] font-[600] text-[var(--text)] mb-1">Session saved on this device</p>
                  <p className="text-[0.92rem] text-[var(--text3)] leading-relaxed">
                    The original images and cluster data are available. Reopen to re-export or make changes.
                  </p>
                  {reopenError && <p className="text-[0.87rem] text-red-500 mt-2">{reopenError}</p>}
                </div>
            ...
            <div className="card p-5 mb-6">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-[1.02rem] font-[600] text-[var(--text)] mb-1">Cluster data saved</p>
                  <p className="text-[0.92rem] text-[var(--text3)] leading-relaxed">
            ...
            <div className="card p-5 mb-6">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-[1.02rem] font-[600] text-[var(--text)] mb-1">Export record</p>
                  <p className="text-[0.92rem] text-[var(--text3)] leading-relaxed">
  ...
  if (!job) {
    return (
      <div>
        ...
        <div className="p-7">
          <p className="text-[1.02rem] text-[var(--text3)]">Job not found.</p>
        </div>
      </div>
    )
  }
      ...
      <div>
        ...
        <div className="p-7">
          <div className="flex items-center gap-3 mb-2">
            <h1 className="text-[1.57rem] font-[600] tracking-[-0.5px] text-[var(--text)]">{job?.name}</h1>
            <span className="chip chip-warning">Ready for Review</span>
          </div>
          <p className="text-[0.95rem] text-[var(--text3)] mb-6">
            {job?.total_images} images · {STEP_LABELS[job?.pipeline_step ?? 0]}
          </p>
          <div className="card p-6 flex items-center gap-4">
            <div className="flex-1">
              <p className="text-[0.97rem] font-[500] text-[var(--text)] mb-1">Clusters are ready to review</p>
              <p className="text-[0.95rem] text-[var(--text3)]">Verify SKU assignments and confirm cluster groupings before generating your export.</p>
            </div>
      ...
      <div className="p-7">
        ...
        {job?.status === 'error' && (
          <div className="flex items-start gap-3 px-4 py-3 rounded-sm bg-[rgba(232,122,122,0.08)] border border-[rgba(232,122,122,0.2)] mb-6">
            ...
            <p className="text-[0.89rem] text-[var(--text)]">
        ...
        {job?.status !== 'error' && (
          <div className="card mb-6">
            <div className="card-head">
              ...
              <span className="text-[0.92rem] text-[var(--text2)]" style={{ fontFamily: 'var(--font-dm-mono)' }}>
