...
function SettingsInner() {
  ...
  return (
    <div>
      ...
      <div className="p-7">
        <div className="mb-7">
          <h1 className="text-[1.67rem] font-[700] tracking-[-0.5px] text-[var(--text)]" style={{ fontFamily: 'var(--font-syne)' }}>Settings</h1>
          <p className="text-[0.95rem] text-[var(--text2)] mt-[6px]">Manage your account, team, and billing.</p>
        </div>

        <div className="flex items-center gap-3 mb-6 flex-wrap">
          <div className="inline-flex bg-[var(--bg3)] p-[3px] rounded-sm gap-[2px]">
            {TABS.map((t) => (
              <button key={t.id} onClick={() => setTab(t.id)} className={`px-[14px] py-[6px] rounded-[5px] text-[0.87rem] font-medium transition-all duration-150 ${tab === t.id ? 'bg-[var(--bg)] text-[var(--text)]' : 'text-[var(--text2)] hover:text-[var(--text)]'}`}>
                {t.label}
              </button>
            ))}
          </div>
          {isAdmin && (
            <a href="/dashboard/admin" className="inline-flex items-center gap-[6px] px-[12px] py-[6px] rounded-[5px] text-[0.92rem] font-medium text-[var(--text3)] hover:text-[var(--text)] border border-[var(--line)] hover:border-[var(--line2)] transition-all">
              ...
              <div className="card-body">
                <div className="flex items-center justify-between py-[14px] border-b border-[var(--line)]">
                  <div>
                    ...
                    <p className="text-[0.87rem] text-[var(--text3)] mt-[2px]">Shown in the sidebar and on exports</p>
                  </div>
        ...
        {tab === 'billing' && (
          <div className="p-7 pt-0 flex flex-col gap-4 max-w-[760px]">
            {activatingPlan && (
              <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-[rgba(255,159,10,0.3)] bg-[rgba(255,159,10,0.07)] text-[0.92rem]">
            ...
            {planActivated && (
              <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-[rgba(48,209,88,0.3)] bg-[rgba(48,209,88,0.07)] text-[0.92rem] text-[var(--text2)]">
              ...
              <div className="card-head">
                ...
                {planId !== 'free' && process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY && (
                  <div className="flex items-center gap-3">
                    <button onClick={handleBillingPortal} disabled={portalLoading} className="text-[0.87rem] text-[var(--text3)] hover:text-[var(--text2)] transition-colors">{portalLoading ? 'Loading…' : 'Manage subscription →'}</button>
                  </div>
                )}
              </div>
              <div className="card-body">
                <div className="flex items-center justify-between py-1">
                  <div className="flex items-center gap-3">
                    <div className={`px-3 py-[5px] rounded-sm text-[0.92rem] font-bold uppercase tracking-[0.05em] ${planId === 'enterprise' ? 'bg-[rgba(232,122,122,0.15)] text-[var(--accent3)]' : planId === 'scale' ? 'bg-[rgba(122,180,232,0.15)] text-[var(--accent4)]' : planId === 'growth' ? 'bg-[rgba(232,217,122,0.15)] text-[var(--accent)]' : planId === 'launch' ? 'bg-[rgba(62,207,142,0.15)] text-[var(--accent2)]' : 'bg-[var(--bg3)] text-[var(--text3)]'}`}>{plan.name}</div>
                    <div>
                      <p className="text-[0.95rem] font-semibold text-[var(--text)]">{plan.priceAud === 0 ? 'Free forever' : `$${plan.priceAud} AUD/month`}</p>
                      <p className="text-[0.87rem] text-[var(--text3)]">{plan.description}</p>
                    </div>
                  </div>
            ...
            <div className="card">
              <div className="card-head"><span className="card-title">Usage</span><span className="text-[0.87rem] text-[var(--text3)]">Resets 1st of each month</span></div>
              ...
              <div className="card">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse">
                    <thead>
                      <tr>
                        <th className="text-left text-[0.92rem] font-medium uppercase tracking-[0.08em] text-[var(--text3)] px-3 py-2 border-b border-[var(--line)]">Feature</th>
                        {(['free', 'launch', 'growth', 'scale', 'enterprise'] as const).map((id) => (
                          <th key={id} className={`text-center text-[0.92rem] font-medium uppercase tracking-[0.08em] px-3 py-2 border-b border-[var(--line)] ${id === planId ? 'text-[var(--accent)]' : 'text-[var(--text3)]'}`}>{PLANS[id].name}{id === planId && ' ✓'}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {[
                        ...
                        <tr key={row.feature} className="hover:bg-[var(--bg3)] transition-colors">
                          <td className="px-3 py-[9px] text-[0.87rem] text-[var(--text2)] border-b border-[var(--line)]">{row.feature}</td>
                          {row.values.map((v, i) => {
                            const id = (['free', 'launch', 'growth', 'scale', 'enterprise'] as const)[i]
                            return <td key={id} className={`px-3 py-[9px] text-[0.87rem] text-center border-b border-[var(--line)] ${id === planId ? 'font-semibold text-[var(--text)]' : 'text-[var(--text3)]'}`} style={v === '✓' ? { color: 'var(--accent2)' } : {}}>{v}</td>
                          })}
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="px-3 py-3 border-t border-[var(--line)] flex justify-end">
                  <button
                    onClick={() => openUpgrade(planId === 'free' ? 'Upgrade to unlock more features' : 'change-plan')}
          ...
          <div className="p-7 pt-0 flex flex-col gap-4 max-w-[760px]">
            <div className="card">
              ...
              <div className="card-body flex flex-col gap-3">
                ...
                {inviteResult?.error && <p className="text-[0.92rem] text-[var(--accent3)]">{inviteResult.error}</p>}
                {inviteResult?.url && (
                  <div className="bg-[var(--bg3)] rounded-sm p-3 text-[0.92rem]">
            ...
            <div className="card">
              <div className="card-head"><span className="card-title">Members</span><span className="text-[0.87rem] text-[var(--text3)]">{teamMembers.length} member{teamMembers.length !== 1 ? 's' : ''}</span></div>
              <div className="card-body p-0">
                {!teamLoaded ? <p className="text-[0.87rem] text-[var(--text3)] px-4 py-3">Loading…</p> : teamMembers.length === 0 ? <p className="text-[0.87rem] text-[var(--text3)] px-4 py-3">No members yet.</p> : (
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-[var(--line)]">
                        <th className="text-left text-[0.92rem] font-medium uppercase tracking-[0.06em] text-[var(--text3)] px-4 py-2">User</th>
                        <th className="text-left text-[0.92rem] font-medium uppercase tracking-[0.06em] text-[var(--text3)] px-4 py-2">Role</th>
                        <th className="text-left text-[0.92rem] font-medium uppercase tracking-[0.06em] text-[var(--text3)] px-4 py-2">Joined</th>
                        <th />
                      </tr>
                    </thead>
                    <tbody>
                      {teamMembers.map((m) => (
                        <tr key={m.user_id} className="border-b border-[var(--line)] last:border-0">
                          <td className="px-4 py-[10px] text-[0.87rem] text-[var(--text)] truncate max-w-[200px]">{m.email || m.user_id.slice(0, 8) + '…'}</td>
                          ...
                          <td className="px-4 py-[10px] text-[0.92rem] text-[var(--text3)]">{new Date(m.joined_at).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
                          <td className="px-4 py-[10px] text-right">
                            {(orgRole === 'owner' || orgRole === 'admin') && m.role !== 'owner' && m.user_id !== currentUserId && (
                              <button
                                ...
                                className="text-[0.87rem] text-[var(--text3)] hover:text-[var(--accent3)] transition-colors"
                              >
            ...
            {pendingInvites.length > 0 && (
              <div className="card">
                <div className="card-head"><span className="card-title">Pending Invites</span><span className="text-[0.87rem] text-[var(--text3)]">{pendingInvites.length} pending</span></div>
                <div className="card-body p-0">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-[var(--line)]">
                        <th className="text-left text-[0.92rem] font-medium uppercase tracking-[0.06em] text-[var(--text3)] px-4 py-2">Email</th>
                        <th className="text-left text-[0.92rem] font-medium uppercase tracking-[0.06em] text-[var(--text3)] px-4 py-2">Role</th>
                        <th className="text-left text-[0.92rem] font-medium uppercase tracking-[0.06em] text-[var(--text3)] px-4 py-2">Expires</th>
                        <th />
                      </tr>
                    </thead>
                    <tbody>
                      {pendingInvites.map((inv) => (
                        <tr key={inv.id} className="border-b border-[var(--line)] last:border-0">
                          <td className="px-4 py-[10px] text-[0.87rem] text-[var(--text)]">{inv.email}</td>
                          <td className="px-4 py-[10px]"><span className="chip chip-uploading">{inv.role}</span></td>
                          <td className="px-4 py-[10px] text-[0.92rem] text-[var(--text3)]">{new Date(inv.expires_at).toLocaleDateString('en-AU', { day: 'numeric', month: 'short' })}</td>
                          <td className="px-4 py-[10px] text-right">
                            ...
                            }} className="text-[0.87rem] text-[var(--text3)] hover:text-[var(--accent3)] transition-colors">Revoke</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
