...
function IntegrationsInner() {
  ...
  const { isReady, clusters, jobName } = useSession()
  ...
  const handleShopifyUpload = async (replace: boolean, brand: Brand) => {
    ...
    setShopifyUploading(true); setShopifyDone(false); setShopifyResults([])
    ...
    for (let ci = 0; ci < confirmedClusters.length; ci++) {
      const cluster = confirmedClusters[ci]; const seq = ci + 1; const images = []
      for (let ii = 0; ii < cluster.images.length; ii++) {
        const img = cluster.images[ii]
        setShopifyProgress({ done: doneCount, total: totalImages, phase: `Processing ${cluster.sku || cluster.label} · ${ii + 1}/${cluster.images.length}` })
        try {
          const buffer = await processImageForShopify(img.file, rule.image_dimensions.width, rule.image_dimensions.height, rule.background_color)
          const filename = applyNamingTemplate(template, { brand: brandCode, seq, sku: cluster.sku, color: cluster.color, view: img.viewLabel, index: ii + 1 }) + '.jpg'
          const base64 = btoa(new Uint8Array(buffer).reduce((d, b) => d + String.fromCharCode(b), ''))
          images.push({ filename, base64, position: ii + 1 })
        } catch (err) { console.warn(`Skipped: ${img.filename}`, err) }
        doneCount++
    ...
    try {
      const { createClient } = await import('@/lib/supabase/client')
      const { data: { session } } = await createClient().auth.getSession()
      const res = await fetch('/api/shopify/upload', { method: 'POST', headers: { 'Content-Type': 'application/json', ...(session?.access_token ? { Authorization: `Bearer ${session.access_token}` } : {}) }, body: JSON.stringify({ brand_id: brand.id, clusters: clustersPayload, replace }) })
      const json = await res.json()
      setShopifyResults(json.data?.results ?? [])
    } catch (err) { console.error('Shopify upload failed:', err) }
    setShopifyUploading(false); setShopifyDone(true)
  }
    ...
    <div>
      ...
      {connectModal && modalMeta && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setConnectModal(null)}>
          <div className="bg-[var(--bg2)] border border-[var(--line)] rounded-[14px] shadow-2xl w-full max-w-[400px] mx-4 p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-[0.78rem] text-[var(--text3)] mb-4">You&apos;ll be redirected to {connectModal === 'google_drive' ? 'Google' : 'Dropbox'} to sign in and approve access. ShotSync will be able to:</p>
            ...
            <p className="text-[0.78rem] text-[var(--text3)] mb-5">You can disconnect at any time from this page.</p>
      ...
      <div className="p-7">
        <div className="mb-7">
          ...
          <p className="text-[0.88rem] text-[var(--text2)] mt-[6px]">Connect platforms, configure cloud storage, and set marketplace export rules.</p>
        </div>
        ...
        <div className="flex flex-col gap-10 max-w-[760px]">
          ...
          {/* ── Shopify ──────────────────────────────────────────────────── */}
          <section>
            <h2 className="text-[1rem] font-semibold text-[var(--text)] mb-4" style={{ fontFamily: 'var(--font-syne)' }}>Shopify</h2>
            <div className="flex flex-col gap-4">

              <div className="card">
                <div className="card-head">
                  <span className="card-title">Connected Brands</span>
                  <p className="text-[0.8rem] text-[var(--text3)]">Shopify credentials are configured per brand in <a href="/dashboard/brands" className="text-[var(--accent)] hover:underline">Brands</a>.</p>
                </div>
                <div className="card-body">
                  {brands.length === 0 ? (
                    <p className="text-[0.8rem] text-[var(--text3)]">No brands yet. <a href="/dashboard/brands" className="text-[var(--accent)] hover:underline">Add a brand</a> to configure Shopify credentials.</p>
                  ) : (
                      ...
                      {brands.map((b) => {
                        ...
                        return (
                          <div key={b.id} className="flex items-center gap-3 py-[10px] border-b border-[var(--line)] last:border-0">
                            <div className="w-7 h-7 rounded-sm flex items-center justify-center text-[0.83rem] font-bold flex-shrink-0" style={{ background: b.logo_color, color: '#000' }}>{b.brand_code.slice(0, 2)}</div>
                            <div className="flex-1 min-w-0">
                              <p className="text-[0.78rem] font-medium text-[var(--text)] truncate">{b.name}</p>
                              <p className="text-[0.78rem] text-[var(--text3)] truncate">{connected ? b.shopify_store_url : 'No Shopify credentials'}</p>
                            </div>
                            <div className={`flex items-center gap-1 text-[0.78rem] font-medium ${connected ? 'text-[var(--accent2)]' : 'text-[var(--text3)]'}`}>
                              <div className={`w-[6px] h-[6px] rounded-full ${connected ? 'bg-[var(--accent2)]' : 'bg-[var(--bg4)]'}`} />
                              {connected ? 'Connected' : 'Not connected'}
                            </div>
                            <a href="/dashboard/brands" className="btn btn-ghost btn-sm">Edit</a>
                          </div>
                        )
                      })}
                    </div>
              ...
              <div className="card">
                <div className="card-head">
                  ...
                  <p className="text-[0.8rem] text-[var(--text3)]">Upload processed images from the current job to matching Shopify products by SKU.</p>
                </div>
                <div className="card-body">
                  {(() => {
                    const shopifyBrands = brands.filter((b) => b.shopify_store_url && b.shopify_access_token)
                    if (shopifyBrands.length === 0) {
                      return <p className="text-[0.8rem] text-[var(--text3)]">No brands have Shopify credentials configured. <a href="/dashboard/brands" className="text-[var(--accent)] hover:underline">Edit a brand</a> to add your store domain and access token.</p>
                    }
                    if (!isReady || confirmedClusters.length === 0) {
                      return (
                        <div className="flex flex-col gap-2">
                          <p className="text-[0.8rem] text-[var(--text3)]">{isReady ? 'No confirmed clusters in the current job. Go to Review and confirm at least one look to enable push.' : 'No active job. Upload and review images first, then return here to push to Shopify.'}</p>
                    ...
                    const shopifyBrand = shopifyBrands.find((b) => b.id === effectiveBrandId) ?? shopifyBrands[0]
                    const pct = shopifyProgress.total > 0 ? Math.round((shopifyProgress.done / shopifyProgress.total) * 100) : 0
                    return (
                      <div className="flex flex-col gap-4">
                        <div className="text-[0.8rem] text-[var(--text2)]">
                          <span className="text-[var(--accent2)] font-semibold">{confirmedClusters.length}</span> confirmed clusters · <span className="text-[var(--text)]">{confirmedClusters.reduce((s, c) => s + c.images.length, 0)}</span> images{jobName && <span className="text-[var(--text3)]"> · {jobName}</span>}
                        </div>
                        {shopifyBrands.length > 1 && (
                          <div>
                            <label className="text-[0.78rem] text-[var(--text2)] mb-[6px] block">Brand</label>
                            <select className="input" value={effectiveBrandId} onChange={(e) => setSelectedShopifyBrandId(e.target.value)}>
                              {shopifyBrands.map((b) => <option key={b.id} value={b.id}>{b.name} — {b.shopify_store_url}</option>)}
                            </select>
                          </div>
                        )}
                          ...
                          <div className="flex flex-col gap-1">
                            {shopifyResults.map((r) => (
                              <div key={r.sku} className="flex items-center gap-2 text-[0.78rem]">
                                {r.status === 'uploaded' ? <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="var(--accent2)" strokeWidth="2"><polyline points="2 6 5 9.5 10 2.5"/></svg> : <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="var(--accent3)" strokeWidth="2"><path d="M2 2l8 8M10 2L2 10"/></svg>}
                        ...
                        <div className="flex items-center gap-2 pt-1">
                          <button onClick={() => handleShopifyUpload(false, shopifyBrand)} disabled={shopifyUploading} className="btn btn-ghost">{shopifyUploading ? 'Uploading…' : 'Add images'}</button>
                          <button onClick={() => handleShopifyUpload(true, shopifyBrand)} disabled={shopifyUploading} className="btn btn-primary">{shopifyUploading ? 'Uploading…' : 'Replace & upload'}</button>
                          {shopifyDone && !shopifyUploading && <button onClick={() => { setShopifyDone(false); setShopifyResults([]) }} className="btn btn-ghost btn-sm">Reset</button>}
                        </div>
                      </div>
                    )
                  })()}
          ...
          <section>
            ...
            <p className="text-[0.78rem] text-[var(--text3)] mb-5">Changes save automatically to your browser.</p>
                ...
                return (
                  <div key={id} className="card">
                    <div className="card-head">
                      ...
                      <button onClick={() => resetRule(id)} className="text-[0.78rem] text-[var(--text3)] hover:text-[var(--text2)] transition-colors">Reset to default</button>
                    </div>
                    <div className="card-body">
                      <div className="grid grid-cols-2 gap-x-8 gap-y-0">
                        ...
                        <SRow label="JPEG Quality" sub={`${rule.quality}% — affects file size`}>
                          <div className="flex items-center gap-3 w-[180px]">
                            ...
                            <span className="text-[0.78rem] w-8 text-right text-[var(--text)]" style={{ fontFamily: 'var(--font-dm-mono)' }}>{rule.quality}%</span>
                          </div>
                        </SRow>
                      ...
                      <div className="mt-3 pt-3 border-t border-[var(--line)] flex items-center gap-3">
                        <span className="text-[0.78rem] text-[var(--text3)] flex-shrink-0">Preview:</span>
                ...
                <div className="flex items-center justify-between py-3 border-b border-[var(--line)]">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {(cc as Record<string, Record<string, string> | undefined>).dropbox?.account_email ? <p className="text-[0.78rem] text-[#1dc44a]">Connected · {(cc as Record<string, Record<string, string> | undefined>).dropbox?.account_email}</p> : <p className="text-[0.78rem] text-[var(--text3)]">Browse and export to Dropbox folders</p>}
                    </div>
                  </div>
                  {(cc as Record<string, unknown>).dropbox ? (
                    ...
                    <button onClick={() => { if (selectedBrand) setConnectModal('dropbox') }} disabled={!dropboxEnabled} className="btn btn-ghost text-[0.78rem]" title={!dropboxEnabled ? 'Set NEXT_PUBLIC_DROPBOX_APP_KEY to enable' : ''}>{dropboxEnabled ? 'Connect' : 'Not configured'}</button>
                  )}
                </div>
                {/* Google Drive */}
                <div className="flex items-center justify-between py-3 border-b border-[var(--line)]">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {(cc as Record<string, Record<string, string> | undefined>).google_drive?.email ? <p className="text-[0.78rem] text-[#1dc44a]">Connected · {(cc as Record<string, Record<string, string> | undefined>).google_drive?.email}</p> : <p className="text-[0.78rem] text-[var(--text3)]">Browse and export to Google Drive folders</p>}
                    </div>
                  </div>
                  {(cc as Record<string, unknown>).google_drive ? (
                    ...
                    <button onClick={() => { if (selectedBrand) setConnectModal('google_drive') }} disabled={!googleEnabled} className="btn btn-ghost text-[0.78rem]" title={!googleEnabled ? 'Set NEXT_PUBLIC_GOOGLE_CLIENT_ID to enable' : ''}>{googleEnabled ? 'Connect' : 'Not configured'}</button>
                  )}
                </div>
                {/* AWS S3 */}
                <div>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      {(cc as Record<string, Record<string, string> | undefined>).s3?.bucket ? <p className="text-[0.78rem] text-[#1dc44a]">Connected · {(cc as Record<string, Record<string, string> | undefined>).s3?.bucket}</p> : <p className="text-[0.78rem] text-[var(--text3)]">Direct upload/download from your S3 bucket</p>}
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2 ml-11">
                    <div className="col-span-2 grid grid-cols-2 gap-2">
                      <div><label className="text-[0.78rem] text-[var(--text3)] mb-1 block">Bucket name *</label><input className="input text-[0.8rem]" placeholder="my-brand-assets" value={s3Form.bucket} onChange={(e) => setS3Form((f) => ({ ...f, bucket: e.target.value }))} /></div>
                      <div><label className="text-[0.78rem] text-[var(--text3)] mb-1 block">Region *</label><input className="input text-[0.8rem]" placeholder="ap-southeast-2" value={s3Form.region} onChange={(e) => setS3Form((f) => ({ ...f, region: e.target.value }))} /></div>
                    </div>
                    <div><label className="text-[0.78rem] text-[var(--text3)] mb-1 block">Access Key ID *</label><input className="input text-[0.8rem]" placeholder="AKIAIOSFODNN7EXAMPLE" value={s3Form.access_key_id} onChange={(e) => setS3Form((f) => ({ ...f, access_key_id: e.target.value }))} autoComplete="off" /></div>
                    <div><label className="text-[0.78rem] text-[var(--text3)] mb-1 block">Secret Access Key *</label><input className="input text-[0.8rem]" type="password" placeholder="••••••••••••••••••••••••••••••••" value={s3Form.secret_access_key} onChange={(e) => setS3Form((f) => ({ ...f, secret_access_key: e.target.value }))} autoComplete="new-password" /></div>
                    <div className="col-span-2"><label className="text-[0.78rem] text-[var(--text3)] mb-1 block">Key prefix <span className="text-[var(--text3)]">(optional — e.g. shoots/)</span></label><input className="input text-[0.8rem]" placeholder="shoots/" value={s3Form.prefix} onChange={(e) => setS3Form((f) => ({ ...f, prefix: e.target.value }))} /></div>
                    {s3Error && <p className="col-span-2 text-[0.8rem] text-[#ff3b30]">{s3Error}</p>}
                    <div className="col-span-2 flex items-center gap-3">
                      <button onClick={saveS3} disabled={s3Saving} className="btn btn-primary text-[0.78rem]">{s3Saving ? 'Saving…' : s3Saved ? 'Saved ✓' : 'Save S3 config'}</button>
          ...
          <section>
            <h2 className="text-[1rem] font-semibold text-[var(--text)] mb-4" style={{ fontFamily: 'var(--font-syne)' }}>Platform Integrations</h2>
            <div className="card">
              <div className="card-body">
                <p className="text-[0.8rem] text-[var(--text3)] mb-4">Direct API integrations with marketplace partners are coming soon. Once enabled, ShotSync will push approved assets directly to each platform — no manual export required.</p>
                <div className="flex flex-col gap-3">
                  {[
                    ...
                    <div key={mp.name} className="flex items-center justify-between px-4 py-3 rounded-sm bg-[var(--bg3)] border border-[var(--line)]">
                      <div>
                        <p className="text-[1rem] font-medium text-[var(--text)]">{mp.name}</p>
                        <p className="text-[0.8rem] text-[var(--text3)] mt-[2px]">{mp.desc}</p>
                      </div>
                      {'available' in mp ? <span className="chip chip-done text-[0.86rem] ml-4 flex-shrink-0">Available</span> : <span className="chip chip-uploading text-[0.86rem] ml-4 flex-shrink-0">Coming soon</span>}
                    </div>
                  ))}
                </div>
              </div>
...
async function processImageForShopify(file: File, width: number, height: number, bgColor: string): Promise<ArrayBuffer> {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      ctx.fillStyle = bgColor || '#ffffff'; ctx.fillRect(0, 0, width, height)
      const srcAspect = img.width / img.height; const dstAspect = width / height
