...
export default function IntegrationsPage() {
  ...
  return (
    <div>
      ...
      <div className="p-7">
        <div className="mb-7">
          <h1 className="text-[length:var(--font-3xl)] font-[700] tracking-[-0.5px] text-[var(--text)]" style={{ fontFamily: 'var(--font-syne)' }}>Integrations</h1>
          <p className="text-[length:var(--font-base)] text-[var(--text2)] mt-[6px]">Manage platform connections for each brand. Connect once and push images directly from any export.</p>
        </div>

        {brands.length === 0 ? (
          <div className="card max-w-[560px]">
            <div className="card-body">
              <p className="text-[length:var(--font-base)] text-[var(--text3)]">No brands yet. <a href="/dashboard/brands" className="text-[var(--accent)] hover:underline">Add a brand</a> to configure platform integrations.</p>
            </div>
          </div>
        ) : (
  ...
  return (
    <div className="card overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-[var(--line)]" style={{ borderLeft: `3px solid ${brand.logo_color}` }}>
        <div className="w-8 h-8 rounded-[6px] flex items-center justify-center text-[length:var(--font-base)] font-bold flex-shrink-0" style={{ background: brand.logo_color, color: '#000' }}>
          {brand.brand_code.slice(0, 2)}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-[length:var(--font-md)] font-semibold text-[var(--text)] leading-tight">{brand.name}</p>
          <p className="text-[length:var(--font-sm)] text-[var(--text3)] font-mono">{brand.brand_code}</p>
        </div>
      ...
      <div className="grid grid-cols-4 divide-x divide-[var(--line)]">
        ...
        <div className="flex flex-col gap-3 p-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[length:var(--font-sm)] font-semibold text-[var(--text)]">Shopify</span>
          </div>

          <div className="flex-1">
            <div className={`flex items-center gap-1.5 text-[length:var(--font-sm)] font-medium mb-1 ${connected ? 'text-[var(--accent2)]' : urlSaved ? 'text-[#ff9f0a]' : 'text-[var(--text3)]'}`}>
            ...
            <p className="text-[length:var(--font-sm)] text-[var(--text3)] truncate leading-snug">
              {urlSaved ? brand.shopify_store_url : 'No store configured'}
            </p>
          </div>

          <a href="/dashboard/brands" className="btn btn-ghost btn-sm text-[length:var(--font-sm)] self-start">
        ...
        {comingSoon.map((market) => (
          <div key={market.id} className="flex flex-col gap-3 p-4 opacity-50">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[length:var(--font-sm)] font-semibold text-[var(--text)] leading-tight">{market.name}</span>
            </div>

            <div className="flex-1">
              <div className="flex items-center gap-1.5 text-[length:var(--font-sm)] text-[var(--text3)] mb-1">
                <div className="w-[5px] h-[5px] rounded-full bg-[var(--bg4)] flex-shrink-0" />
                Coming soon
              </div>
              <p className="text-[length:var(--font-sm)] text-[var(--text3)] leading-snug">{market.api}</p>
            </div>

            <button disabled className="btn btn-ghost btn-sm text-[length:var(--font-sm)] self-start cursor-not-allowed">Connect</button>
          </div>
        ))}
      </div>
    </div>
