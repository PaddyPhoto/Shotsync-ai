...
const ANGLE_OPTIONS = ['full-length', 'front', 'back', 'side', 'full-length-side', 'full-length-back', 'front-3/4', 'back-3/4', 'detail', 'mood', 'mood-2', 'mood-3', 'ghost-mannequin', 'flat-lay', 'top-down', 'inside']
const ALL_ON_MODEL = ['full-length', 'front', 'side', 'mood', 'detail', 'back', 'mood-2', 'mood-3', 'full-length-side', 'full-length-back']
            ...
            {step === 3 && (
              <div className="px-6 pb-5">
                ...
                <div className="mb-6 pb-6 border-b border-[var(--line)]">
                  ...
                  <p className="text-[length:var(--font-base)] text-[var(--text3)] mb-3">Where the GM image is embedded in the live product listing — most relevant for Shopify auto-publish.</p>
                  <div className="inline-flex bg-[var(--bg3)] p-[3px] rounded-sm gap-[2px]">
                    {(['first', 'last'] as const).map((pos) => (
                      <button key={pos} type="button" onClick={() => onFormChange({ gm_position: pos })}
                        ...
                        {pos === 'first' ? 'Before on-model' : 'After on-model'}
                      </button>
                    ))}
                  </div>
                </div>
