    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.75)' }}>
      <div className="bg-[var(--bg)] border border-[var(--line2)] rounded-md w-[560px] max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="flex items-start justify-between px-6 pt-6 pb-4">
          <div>
            <p className="text-[0.86rem] text-[var(--accent)] uppercase tracking-[0.1em] font-semibold mb-1">
              {isChangingPlan ? 'Change Plan' : 'Upgrade Required'}
            </p>
            <h2 className="text-[1.27rem] font-[700] tracking-[-0.3px] text-[var(--text)]" style={{ fontFamily: 'var(--font-syne)' }}>
        ...
        <div className="px-6 pb-6 grid grid-cols-2 gap-4">
          {upgradePlans.map((id) => {
            ...
            return (
              ...
              >
                {isRecommended && (
                  <span className="absolute -top-[10px] left-4 text-[0.79rem] font-bold uppercase tracking-[0.1em] bg-[var(--accent)] text-black px-2 py-[2px] rounded-full">
                    Recommended
                  </span>
                )}
                <div>
                  <p className="text-[1.02rem] font-[700] text-[var(--text)]" style={{ fontFamily: 'var(--font-syne)' }}>
                    {p.name}
                  </p>
                  <p className="text-[0.92rem] text-[var(--text3)] mt-[2px]">{p.description}</p>
                </div>

                <div>
                  {id === 'enterprise' ? (
                    <span className="text-[1.27rem] font-[700] text-[var(--text)]">Contact us</span>
                  ) : (
                    <>
                      {!isChangingPlan && (
                        <div className="inline-flex items-center gap-1.5 bg-[rgba(62,207,142,0.12)] text-[var(--accent2)] text-[0.84rem] font-semibold px-2 py-[3px] rounded-full mb-2">
                          30 days free
                        </div>
                      )}
                      <div className="flex items-baseline gap-1">
                        <span className="text-[1.67rem] font-[700] text-[var(--text)]">${p.priceAud}</span>
                        <span className="text-[0.89rem] text-[var(--text3)]">AUD/month{!isChangingPlan && ' after trial'}</span>
                      </div>
                    </>
                  )}
                </div>
                ...
                <ul className="flex flex-col gap-[6px]">
                  {p.highlights.map((h) => (
                    <li key={h} className="flex items-center gap-2 text-[0.92rem] text-[var(--text2)]">
                ...
                {!STRIPE_CONFIGURED && (
                  <p className="text-[0.79rem] text-[var(--text3)] text-center -mt-2">
        ...
        <div className="border-t border-[var(--line)] px-6 py-3 flex items-center justify-between flex-wrap gap-3">
          <button onClick={closeUpgrade} className="text-[0.92rem] text-[var(--text3)] hover:text-[var(--text2)] transition-colors">
