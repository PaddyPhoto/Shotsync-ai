...
function UpdateForm({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {error && (
        <div style={{ background: 'rgba(255,59,48,0.08)', border: '0.5px solid rgba(255,59,48,0.2)', borderRadius: '8px', padding: '10px 14px', fontSize: 'var(--font-base)', color: '#c0392b' }}>
          {error}
        </div>
      )}
      <div style={{ display: 'flex', gap: '10px' }}>
        <button
          type="button"
          onClick={onClose}
          style={{ flex: 1, padding: '11px', background: 'rgba(0,0,0,0.06)', color: '#1d1d1f', border: 'none', borderRadius: '8px', fontSize: 'var(--font-base)', fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}
        >
          Cancel
        </button>
        <button
          ...
          style={{ flex: 2, padding: '11px', background: loading ? 'rgba(0,0,0,0.4)' : '#1d1d1f', color: '#fff', border: 'none', borderRadius: '8px', fontSize: 'var(--font-base)', fontWeight: 500, cursor: loading ? 'wait' : 'pointer', fontFamily: 'inherit' }}
        >
    ...
    >
      ...
      >
        ...
        <div style={{ padding: '20px 24px 16px', borderBottom: '0.5px solid rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <p style={{ fontSize: 'var(--font-lg)', fontWeight: 500, color: '#1d1d1f', letterSpacing: '-.2px', margin: 0 }}>Update payment method</p>
          <button
            onClick={onClose}
        ...
        <div style={{ padding: '24px' }}>
          {success ? (
            <div style={{ textAlign: 'center', padding: '24px 0' }}>
              ...
              <p style={{ fontSize: 'var(--font-lg)', fontWeight: 500, color: '#1d1d1f', marginBottom: '8px' }}>Payment method updated</p>
              <p style={{ fontSize: 'var(--font-base)', color: '#6e6e73', marginBottom: '20px' }}>Your new payment method will be used for future charges.</p>
              <button onClick={onClose} style={{ background: '#1d1d1f', color: '#fff', border: 'none', borderRadius: '8px', padding: '10px 24px', fontSize: 'var(--font-base)', cursor: 'pointer', fontFamily: 'inherit' }}>Done</button>
            </div>
          ) : fetching ? (
            <div style={{ padding: '40px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', color: '#6e6e73', fontSize: 'var(--font-md)' }}>
            ...
            <div style={{ textAlign: 'center', padding: '24px 0' }}>
              <p style={{ fontSize: 'var(--font-base)', color: '#ff3b30', marginBottom: '16px' }}>{error}</p>
              <button onClick={onClose} style={{ background: 'rgba(0,0,0,0.06)', color: '#1d1d1f', border: 'none', borderRadius: '8px', padding: '9px 20px', fontSize: 'var(--font-base)', cursor: 'pointer', fontFamily: 'inherit' }}>Close</button>
            </div>
          ) : clientSecret ? (
        ...
        {!success && !fetching && !error && (
          <div style={{ padding: '12px 24px 16px', borderTop: '0.5px solid rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', gap: '6px', fontSize: 'var(--font-sm)', color: '#aeaeb2' }}>
