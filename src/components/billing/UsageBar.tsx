  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2">
        <span className="text-[0.86rem] text-[var(--text3)] w-[100px] truncate">{label}</span>
        <div className="flex-1 h-[3px] bg-[var(--bg4)] rounded-full overflow-hidden">
          {!unlimited && (
            <div className={`h-full rounded-full transition-all ${barColor}`} style={{ width: `${pct}%` }} />
          )}
        </div>
        <span className="text-[0.84rem] w-[56px] text-right" style={{ fontFamily: 'var(--font-dm-mono)', color: isAtLimit ? 'var(--accent3)' : 'var(--text3)' }}>
