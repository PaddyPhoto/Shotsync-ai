  ...
  return (
    <div className="flex flex-col gap-[6px] mt-5">
      {files.map((item) => (
        <div
          key={item.id}
          className="flex items-center gap-3 bg-[var(--bg3)] border border-[var(--line)] rounded-sm px-[14px] py-[10px] text-[var(--font-sm)]"
        >
          {/* Thumbnail */}
          <div className="w-9 h-9 rounded-[4px] bg-[var(--bg4)] flex-shrink-0 overflow-hidden flex items-center justify-center">
            {item.preview ? (
              ...
              <span
                className="text-[var(--font-xs)] text-[var(--text3)]"
          ...
          <div className="flex-1 min-w-0">
            ...
            {item.status === 'error' && (
              <p className="text-[var(--font-sm)] text-[var(--accent3)] mt-0.5">{item.error}</p>
            )}
          </div>

          {/* Size */}
          <span
            className="text-[var(--text3)] text-[var(--font-sm)] flex-shrink-0"
            style={{ fontFamily: 'var(--font-dm-mono)' }}
          >
