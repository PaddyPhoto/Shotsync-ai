    ...
    <div className="flex flex-col gap-[2px]">
      {steps.map((step) => (
        ...
        >
          ...
          <div
            className={cn(
              'w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center text-[length:var(--font-sm)] font-medium border',
          ...
          <div className="flex-1">
            <p className={cn(
              'text-[length:var(--font-base)] font-medium',
              step.status === 'active' ? 'text-[var(--text)]' : 'text-[var(--text2)]'
            )}>
              {step.name}
            </p>
            <p className="text-[length:var(--font-sm)] text-[var(--text3)] mt-[2px]">
              {step.description}
            </p>
          </div>
          ...
          {step.duration_ms !== undefined && step.status === 'done' && (
            <span
              className="text-[length:var(--font-sm)] text-[var(--text3)] flex-shrink-0"
              style={{ fontFamily: 'var(--font-dm-mono)' }}
            >
