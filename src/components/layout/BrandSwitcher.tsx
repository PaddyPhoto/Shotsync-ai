  ...
  return (
    <div ref={ref} className="relative mx-3 mt-3 mb-1">
      ...
      >
        {/* Brand avatar */}
        <div
          className="w-7 h-7 rounded-[5px] flex items-center justify-center font-bold text-[length:var(--font-base)] text-black flex-shrink-0"
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[length:var(--font-sm)] font-medium text-[var(--text)] truncate leading-tight">
      ...
      {open && (
        <div className="absolute left-0 right-0 top-[calc(100%+4px)] bg-[var(--bg2)] border border-[var(--line2)] rounded-sm shadow-[0_8px_24px_rgba(0,0,0,0.4)] z-50 overflow-hidden">
          <div className="px-3 pt-2 pb-1">
            <p className="text-[length:var(--font-base)] text-[var(--text3)] uppercase tracking-[0.1em] font-medium">Switch brand</p>
          </div>
          ...
          <div className="flex flex-col gap-[2px] px-[6px] pb-[6px]">
            {brands.map((brand) => {
              ...
              return (
                ...
                >
                  <div
                    className="w-6 h-6 rounded-[4px] flex items-center justify-center font-bold text-[length:var(--font-2xs)] text-black flex-shrink-0"
                    style={{ background: brand.logo_color, fontFamily: 'var(--font-dm-mono)' }}
                  >
                    {brand.brand_code}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={cn('text-[length:var(--font-sm)] truncate', isActive ? 'text-[var(--accent)] font-medium' : 'text-[var(--text2)]')}>
                      {brand.name}
                    </p>
                  </div>
                  {isActive && (
          ...
          <div className="border-t border-[var(--line)] px-[6px] py-[6px]">
            <button
              ...
              className="w-full flex items-center gap-2 px-[8px] py-[7px] rounded-[4px] text-[length:var(--font-sm)] text-[var(--text3)] hover:text-[var(--text2)] hover:bg-[var(--bg3)] transition-colors"
            >
