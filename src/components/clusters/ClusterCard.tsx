...
function ConfidenceBar({ value }: { value: number }) {
  ...
  return (
    <div className="flex items-center gap-2 text-[length:var(--font-sm)] text-[var(--text3)]">
  ...
  return (
    <div className={cn(
      ...
      <div className="px-[14px] py-3 bg-[var(--bg3)] border-b border-[var(--line)] flex items-center justify-between">
        <span
          className="text-[length:var(--font-sm)] text-[var(--text3)] bg-[var(--bg4)] px-2 py-[3px] rounded-[4px] border border-[var(--line)]"
      ...
      <div className="flex gap-[2px] p-[10px]">
        {cluster.images.slice(0, 4).map((img) => (
          ...
          >
            ...
            <span className="absolute bottom-1 left-1 bg-[rgba(0,0,0,0.7)] backdrop-blur-[4px] text-[length:var(--font-2xs)] font-semibold uppercase text-[var(--text)] px-[5px] py-[2px] rounded-[3px]">
              {img.view_label}
            </span>
          </div>
        ))}
        {cluster.image_count > 4 && (
          <div className="flex-1 aspect-[3/4] bg-[var(--bg3)] rounded-[4px] flex items-center justify-center border border-dashed border-[var(--line2)] min-w-0">
            <span className="text-[length:var(--font-sm)] text-[var(--text3)] text-center">
              +{cluster.image_count - 4}
            </span>
          </div>
        )}
        {cluster.images.length < 3 && Array.from({ length: 3 - cluster.images.length }).map((_, i) => (
          <div key={`empty-${i}`} className="flex-1 aspect-[3/4] bg-[var(--bg3)] rounded-[4px] border border-dashed border-[var(--line2)] flex items-center justify-center min-w-0">
            <span className="text-[length:var(--font-xs)] text-[var(--text3)] text-center">missing</span>
          </div>
        ))}
      </div>
      ...
      <div className="px-[14px] pb-3">
        <p className="text-[length:var(--font-sm)] text-[var(--text3)] mb-[6px] uppercase tracking-[0.08em]">
          SKU Match
        </p>
        {cluster.suggested_skus.length > 0 ? (
            ...
            {cluster.suggested_skus.slice(0, 3).map((sug) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  <p className="text-[length:var(--font-sm)] text-[var(--text)] truncate">{sug.product_name}</p>
                  <p className="text-[length:var(--font-sm)] text-[var(--text3)]" style={{ fontFamily: 'var(--font-dm-mono)' }}>
                    {sug.sku}
          ...
          <div className="bg-[var(--bg3)] rounded-sm px-3 py-2 text-[length:var(--font-base)] text-[var(--text3)]">
      ...
      <div className="px-[14px] py-3 border-t border-[var(--line)] flex items-center gap-2">
        ...
        {isConfirmed ? (
          <span className="text-[length:var(--font-sm)] font-semibold text-[var(--accent2)] flex items-center gap-1">
